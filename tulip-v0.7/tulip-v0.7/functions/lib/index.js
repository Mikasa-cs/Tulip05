...
export const stripeWebhook = functions.https.onRequest((req, res) => {
    corsHandler(req, res, async () => {
        ...
        let event;
        try {
            event = stripe.webhooks.constructEvent(req.rawBody || Buffer.from(JSON.stringify(req.body)), signature, STRIPE_WEBHOOK_SECRET);
        }
        catch (err) {
 ...
 * Create order from checkout session
 */
async function createOrderFromCheckoutSession(userId, session) {
    try {
