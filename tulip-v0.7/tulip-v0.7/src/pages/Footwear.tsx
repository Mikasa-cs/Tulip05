...
import { products, Gender, Usage } from '@/data/products';
import { useCart } from '@/context/CartContext';

// ── Constants ───────────────────────────────────────────────────────────────
const genderTabs: { id: Gender | 'All'; label: string; image: string; objectPosition?: string }[] = [
  {
    id: 'All',
    label: 'All Footwear',
  ...
  },
  {
    id: 'Boys',
    label: "Boys'",
    image: 'https://images.unsplash.com/photo-1600269452121-4f2416e55c28?w=900&fit=crop&auto=format&q=80',
    objectPosition: 'center',
  },
  {
    id: 'Girls',
    label: "Girls'",
    image: 'https://images.unsplash.com/photo-1560769629-975ec94e6a86?w=900&fit=crop&auto=format&q=80',
    objectPosition: 'center',
  },
];

const footwearHeroBackgroundImage = 'https://images.pexels.com/photos/1240892/pexels-photo-1240892.jpeg';
...
// ── Footwear base set ────────────────────────────────────────────────────────
const allFootwear = products.filter((p) => p.masterCategory === 'Footwear');
...
const Footwear: React.FC = () => {
  ...
  const dataMaxPrice = useMemo(
    () =>
      allFootwear.length > 0
        ? Math.ceil(Math.max(...allFootwear.map((p) => p.price)) / 500) * 500
        : 5000,
    [],
  );

  const filtered = useMemo(() => {
    let list = activeGender === 'All'
      ? [...allFootwear]
      : allFootwear.filter((p) => p.gender === activeGender);
    ...
    return list;
  }, [activeGender, search, selectedUsages, selectedColors, priceMax, sortBy]);
  ...
  const activeFilterCount =
    selectedUsages.length + selectedColors.length + (priceMax < dataMaxPrice ? 1 : 0);
    ...
    <main className="min-h-screen bg-background">
      ...
      <section className="relative isolate pt-32 pb-16 overflow-hidden">
        <div className="absolute inset-0 z-0">
          <img
            src={footwearHeroBackgroundImage}
        ...
        <div className="absolute inset-0 z-[1] overflow-hidden">
          <motion.div
            className="absolute top-10 right-[10%] w-72 h-72 rounded-full bg-primary/8 blur-3xl"
            animate={{ scale: [1, 1.3, 1], opacity: [0.3, 0.6, 0.3] }}
            transition={{ duration: 10, repeat: Infinity }}
          />
          <motion.div
            className="absolute bottom-0 left-[5%] w-96 h-96 rounded-full bg-foreground/4 blur-3xl"
        ...
        <div className="container relative z-10">
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            <p className="font-body text-xs uppercase tracking-[0.35em] text-primary mb-3">Footwear</p>
            <h1 className="font-display text-5xl md:text-7xl font-medium leading-[1.05] mb-5">
              Step Into{' '}
              <span className="text-gradient-tulip italic">Your Style</span>
            </h1>
            <p className="font-body text-muted-foreground text-lg max-w-lg">
              {allFootwear.length.toLocaleString()} styles across heels, sneakers, flats, boots and more — for every step you take.
            </p>
          </motion.div>

          {/* ── Gender subcategory cards ── */}
          <motion.div
          ...
          >
            {genderTabs.map((tab, i) => {
              const count = tab.id === 'All'
                ? allFootwear.length
                : allFootwear.filter((p) => p.gender === tab.id).length;
                ...
                >
                  <div className="relative h-36">
                    <img
                      ...
                      loading="lazy"
                      onError={(event) => {
                        event.currentTarget.src = footwearHeroBackgroundImage;
                      }}
                      className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105"
                      style={{ objectPosition: tab.objectPosition ?? 'center' }}
                    />
                    <div className={`absolute inset-0 transition-all duration-300 ${isActive ? 'bg-primary/40' : 'bg-foreground/30 group-hover:bg-foreground/20'}`} />
                    <div className="absolute inset-0 p-3 flex flex-col justify-end">
                      <p className="font-display text-base text-white leading-tight">{tab.label}</p>
      ...
      <div id="results" className="sticky top-16 z-30 bg-background/95 backdrop-blur-xl border-b border-border/60 shadow-sm">
        <div className="container py-3">
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
            ...
            {/* Gender pills (compact, in bar) */}
            <div className="hidden lg:flex items-center gap-1.5 shrink-0">
              {genderTabs.map((tab) => (
                <button
                  key={tab.id}
                  onClick={() => setActiveGender(tab.id)}
                  className={`px-3 py-1.5 rounded-full text-xs font-body font-medium border transition-all duration-200 whitespace-nowrap ${
                    activeGender === tab.id
                      ? 'bg-primary text-primary-foreground border-primary'
            ...
            {showFilters && (
              ...
              >
                <div className="w-[260px] space-y-7 pr-2">
                  ...
                  {/* Gender (sidebar version for mobile) */}
                  <div>
                    <p className="font-body text-xs uppercase tracking-widest text-muted-foreground mb-3">Gender</p>
                    <div className="flex flex-wrap gap-2">
                      {genderTabs.map((tab) => (
                        <Pill
                          key={tab.id}
                          active={activeGender === tab.id}
                          onClick={() => setActiveGender(tab.id)}
                        >
                          {tab.label}
                        </Pill>
            ...
            <div className="flex items-center justify-between mb-6">
              <div>
                <h2 className="font-display text-2xl font-medium">
                  {activeGender === 'All' ? 'All Footwear' : `${activeGender}'s Footwear`}
                </h2>
                <p className="font-body text-sm text-muted-foreground mt-0.5">{filtered.length.toLocaleString()} styles</p>
              </div>
            </div>
            ...
            {filtered.length === 0 && (
              ...
              >
                ...
                <p className="font-body text-muted-foreground mb-6 max-w-xs">
                  Try adjusting your search or filters to discover more styles.
                </p>
