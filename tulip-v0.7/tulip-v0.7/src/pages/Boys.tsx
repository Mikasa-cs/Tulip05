...
const subCategoryTabs: { id: string; label: string; image: string; match: string[]; objectPosition?: string }[] = [
  {
    id: 'All',
    label: 'All Boys',
    image: 'https://images.pexels.com/photos/1564868/pexels-photo-1564868.jpeg',
  ...
  },
  {
    id: 'Footwear',
    label: 'Footwear',
    image: 'https://images.pexels.com/photos/1909014/pexels-photo-1909014.jpeg',
    objectPosition: 'center',
    match: ['Shoes', 'Flip Flops', 'Sandal'],
  },
];

const boysHeroBackgroundImage = 'https://images.pexels.com/photos/6069961/pexels-photo-6069961.jpeg';

// ── Boys-only base set ───────────────────────────────────────────────────────
const boysProducts = products.filter((p) => p.gender === 'Boys');
  ...
  const dataMaxPrice = useMemo(
    () =>
      boysProducts.length > 0
        ? Math.ceil(Math.max(...boysProducts.map((p) => p.price)) / 500) * 500
        : 5000,
    [],
  );

  const filtered = useMemo(() => {
    let list = [...boysProducts];
    ...
    <main className="min-h-screen bg-background">
      ...
      <section className="relative isolate pt-32 pb-16 overflow-hidden">
        <div className="absolute inset-0 z-0">
          <img
            src={boysHeroBackgroundImage}
            alt="Boys fashion collection"
            loading="eager"
        ...
        <div className="absolute inset-0 z-[1] overflow-hidden">
          <motion.div
            className="absolute top-10 right-[10%] w-72 h-72 rounded-full bg-blue-400/10 blur-3xl"
        ...
        <div className="container relative z-10">
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            <p className="font-body text-xs uppercase tracking-[0.35em] text-primary mb-3">Boys</p>
            <h1 className="font-display text-5xl md:text-7xl font-medium leading-[1.05] mb-5">
              Built for{' '}
              <span className="text-gradient-tulip italic">Adventure</span>
            </h1>
            <p className="font-body text-muted-foreground text-lg max-w-lg">
              {boysProducts.length.toLocaleString()} cool picks — comfy casuals, sporty fits, and everything in between.
            </p>
          </motion.div>
          ...
          >
            {subCategoryTabs.map((tab, i) => {
              const count = tab.id === 'All'
                ? boysProducts.length
                : boysProducts.filter((p) => tab.match.some((m) => p.subCategory.toLowerCase().includes(m.toLowerCase()))).length;
                ...
                >
                  <div className="relative h-32 sm:h-36">
                    <img
                      ...
                      loading="lazy"
                      onError={(event) => {
                        event.currentTarget.src = boysHeroBackgroundImage;
                      }}
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
            ...
            <div className="relative flex-1">
              ...
              <input
                ...
                placeholder="Search boys' fashion…"
