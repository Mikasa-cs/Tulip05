...
const subCategoryTabs: { id: string; label: string; image: string; match: string[]; objectPosition?: string }[] = [
  {
    id: 'All',
    label: 'All Girls',
    image: 'https://images.pexels.com/photos/1140923/pexels-photo-1140923.jpeg',
  ...
  },
  {
    id: 'Bottomwear',
    label: 'Bottomwear',
    image: 'https://images.pexels.com/photos/5253935/pexels-photo-5253935.jpeg',
    objectPosition: 'center top',
    match: ['Bottomwear'],
  },
  {
    id: 'Dress',
    label: 'Dresses',
    image: 'https://images.pexels.com/photos/10071691/pexels-photo-10071691.jpeg',
    objectPosition: 'center top',
    match: ['Dress'],
  },
  {
    ...
    match: ['Sandal', 'Shoes', 'Flip Flops'],
  },
];

const girlsHeroBackgroundImage = 'https://images.pexels.com/photos/8083846/pexels-photo-8083846.jpeg';

// ── Girls-only base set ──────────────────────────────────────────────────────
const girlsProducts = products.filter((p) => p.gender === 'Girls');
  ...
  const dataMaxPrice = useMemo(
    () =>
      girlsProducts.length > 0
        ? Math.ceil(Math.max(...girlsProducts.map((p) => p.price)) / 500) * 500
        : 5000,
    [],
  );

  const filtered = useMemo(() => {
    let list = [...girlsProducts];
    ...
    <main className="min-h-screen bg-background">
      ...
      <section className="relative isolate pt-32 pb-16 overflow-hidden">
        <div className="absolute inset-0 z-0">
          <img
            src={girlsHeroBackgroundImage}
            alt="Girls fashion collection"
            loading="eager"
        ...
        <div className="absolute inset-0 z-[1] overflow-hidden">
          <motion.div
            className="absolute top-10 right-[10%] w-72 h-72 rounded-full bg-pink-300/15 blur-3xl"
            animate={{ scale: [1, 1.3, 1], opacity: [0.4, 0.7, 0.4] }}
            transition={{ duration: 9, repeat: Infinity }}
          />
          <motion.div
            className="absolute bottom-0 left-[5%] w-96 h-96 rounded-full bg-purple-300/10 blur-3xl"
        ...
        <div className="container relative z-10">
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            <p className="font-body text-xs uppercase tracking-[0.35em] text-primary mb-3">Girls</p>
            <h1 className="font-display text-5xl md:text-7xl font-medium leading-[1.05] mb-5">
              Dress Up,{' '}
              <span className="text-gradient-tulip italic">Dream Big</span>
            </h1>
            <p className="font-body text-muted-foreground text-lg max-w-lg">
              {girlsProducts.length.toLocaleString()} vibrant picks — playful prints, pastel tones, and everyday fun.
            </p>
          </motion.div>
          ...
          >
            {subCategoryTabs.map((tab, i) => {
              const count = tab.id === 'All'
                ...
                : girlsProducts.filter((p) => tab.match.some((m) => p.subCategory.toLowerCase().includes(m.toLowerCase()))).length;
                ...
                >
                  <div className="relative h-32 sm:h-36">
                    <img
                      ...
                      loading="lazy"
                      onError={(event) => {
                        event.currentTarget.src = girlsHeroBackgroundImage;
                      }}
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
            ...
            <div className="relative flex-1">
              ...
              <input
                ...
                placeholder="Search girls' fashion…"
